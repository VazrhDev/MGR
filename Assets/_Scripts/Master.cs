...
public class Master : MonoBehaviour
{
    ...
    public static int GetTimeInSeconds(string currentTime, char formatSymbol)
    {
    ...
    {
        string currentTimee = GetCurrentTime(':');
        return GetTimeInSeconds(currentTimee, ':');
    }
    ...
    public static string SecondsToTime(int secondss, bool needHours = false)
    {
        int hr, mnts, scnds;
        hr = secondss / 3600;
        mnts = (secondss % 3600) / 60;
        scnds = (secondss % 3600) % 60;
        string hrS,mntsS,scndsS;
        if (hr < 10) hrS = "0" + hr;
        else hrS = hr.ToString();
        if (mnts < 10) mntsS = "0" + mnts;
        else mntsS = mnts.ToString();
        if (scnds < 10) scndsS = "0" + scnds;
        else scndsS = scnds.ToString();
        if(needHours)
        {
            return hrS + ":" + mntsS + ":" + scndsS;
        }
        else
        {
            return mntsS + ":" + scndsS;
    ...
    public static void ColoredLog(string color, string message)
    {
